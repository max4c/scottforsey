    ...
    <div>
      ...
      >
        ...
        <motion.div
          ...
          style={{ left: `calc(${pct}% - 7px)`, boxShadow: '0 1px 6px rgba(0,0,0,0.4)' }}
...
export function FullscreenPlayer({ onClose }: FullscreenPlayerProps) {
  const {
    ...
    playAt, removeFromQueue, clearQueue,
  } = useAudioPlayer();

  const [view, setView] = useState<'player' | 'queue'>('player');

  // Swipe-down-to-close (player view only)
  ...
  const onTouchMovePanel = (e: React.TouchEvent) => {
    if (view === 'queue' || startYRef.current === null) return;
          ...
          >
            ...
            <div className="flex items-center justify-between pt-2 pb-1 flex-shrink-0">
              <motion.button
                onClick={onClose}
                whileTap={{ scale: 0.88 }}
                className="w-10 h-10 flex items-center justify-center rounded-full text-white/50 active:text-white active:bg-white/10"
                aria-label="Close"
              >
            ...
            <div className="flex-shrink-0 pb-4">
              ...
              {currentTrack.albumTitle && (
                <p className="text-white/45 text-sm mt-0.5 truncate font-body">
                  {currentTrack.albumTitle}
                </p>
              )}
            </div>
                ...
                <AnimatePresence mode="wait" initial={false}>
                  {isPlaying ? (
                    <motion.svg
                      ...
                      transition={{ duration: 0.1 }}
                      width="26" height="26" viewBox="0 0 24 24" fill="#0B1120"
                    >
            ...
            >
              ...
              <div className="relative flex-1 h-8 flex items-center">
                ...
                <div
                  ...
                  style={{ width: `${volume * 100}%`, background: 'rgba(255,140,66,0.85)' }}
                />
                <input
                  type="range"
                ...
                <div
                  ...
                  style={{ left: `calc(${volume * 100}% - 6px)`, boxShadow: '0 1px 4px rgba(0,0,0,0.4)' }}
                />
              </div>
              ...
              <button
                ...
                className={`relative w-10 h-10 flex items-center justify-center rounded-full ml-1 transition-colors text-white/35 active:text-white`}
                aria-label="Queue"
              >
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 pt-2 pb-4 flex-shrink-0">
              ...
              <div className="flex flex-col items-center">
                <span className="font-display font-bold text-white text-base">Queue</span>
                {upNextCount > 0 && (
                  <span className="text-white/35 text-xs">{upNextCount} up next</span>
                )}
              </div>
              ...
              <div className="w-10 flex items-center justify-end">
                {upNextCount > 0 && (
                  <button
                    onClick={clearQueue}
                    className="text-xs text-white/35 active:text-white transition-colors"
                  >
                    Clear
                  </button>
                )}
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-safe">
              ...
              {currentTrack && (
                <div className="mb-5">
                  ...
                  <div className="flex items-center gap-3 rounded-2xl px-3 py-3" style={{ backgroundColor: 'rgba(255,140,66,0.12)' }}>
                    {/* Animated bars */}
                    <div className="flex items-end gap-0.5 h-4 flex-shrink-0">
                      <motion.div
                        animate={{ height: isPlaying ? ['40%', '100%', '40%'] : '40%' }}
                        transition={{ repeat: Infinity, duration: 0.8 }}
                        className="w-0.5 rounded-full"
                        style={{ backgroundColor: '#FF8C42', minHeight: '2px' }}
                      />
                      <motion.div
                        animate={{ height: isPlaying ? ['70%', '30%', '70%'] : '70%' }}
                        transition={{ repeat: Infinity, duration: 0.8, delay: 0.2 }}
                        className="w-0.5 rounded-full"
                        style={{ backgroundColor: '#FF8C42', minHeight: '2px' }}
                      />
                      <motion.div
                        animate={{ height: isPlaying ? ['50%', '80%', '50%'] : '50%' }}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-display font-semibold text-sm text-[#FF8C42] truncate">
                        {currentTrack.title}
                      </p>
                      {currentTrack.albumTitle && (
                        <p className="text-xs text-white/35 truncate">{currentTrack.albumTitle}</p>
                      )}
                    </div>
              ...
              {upNext.length > 0 ? (
                <div>
                  ...
                  <div className="flex flex-col gap-0.5">
                    {upNext.map((track, i) => {
                      ...
                      return (
                        <div key={`${track.id}-${qIdx}`} className="flex items-center gap-1 group rounded-xl overflow-hidden">
                          <button
                            onClick={() => { playAt(qIdx); setView('player'); }}
                            className="flex items-center gap-3 flex-1 min-w-0 px-3 py-3 text-left active:bg-white/5 transition-colors"
                          >
                            <span className="text-xs text-white/25 tabular-nums w-5 flex-shrink-0 text-right">{i + 1}</span>
                            <div className="flex-1 min-w-0">
                              <p className="font-display font-semibold text-sm text-white truncate">{track.title}</p>
                              {track.albumTitle && (
                                <p className="text-xs text-white/35 truncate">{track.albumTitle}</p>
                              )}
                            </div>
                            <span className="text-xs text-white/35 tabular-nums flex-shrink-0">
                          ...
                          <button
                            ...
                            className="w-8 h-8 flex-shrink-0 flex items-center justify-center text-white/20 active:text-white/60 mr-2 transition-colors"
                ...
                <p className="text-sm text-white/25 text-center py-8">Nothing else queued</p>
              )}

              {/* Bottom spacing for safe area */}
              <div style={{ height: 'max(env(safe-area-inset-bottom, 0px), 30px)' }} />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
